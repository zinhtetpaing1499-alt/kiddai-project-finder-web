...
const STORE_NAME = "line-notifications";
const KEY = "unread-v1";
const MAX_ITEMS = 200;
...
/** Strip sheet marker ` line@` / `line@` so mark-read matches LINE display names. */
...
/** @returns {Promise<LineNotification[]>} */
export async function readNotifications() {
  try {
    ...
    console.log("line-store: read failed", error instanceof Error ? error.message : error);
    return [];
  }
}

/** @param {LineNotification[]} items */
...
/** @param {LineNotification[]} incoming */
...
/**
 * @param {{ ids?: string[]; userIds?: string[]; senderNames?: string[] }} options
 */
export async function markNotificationsRead(options) {
  const ids = new Set((options.ids ?? []).map((value) => value.trim()).filter(Boolean));
  const userIds = new Set((options.userIds ?? []).map((value) => value.trim()).filter(Boolean));
  ...
  const next = existing.map((item) => {
    ...
    if (ids.has(item.id) || userIds.has(item.userId) || nameMatch) {
